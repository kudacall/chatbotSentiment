...
speech(status);
disp(status)
[num txt raw] = xlsread('mlinfo.xls');
...
for wordIndex = 1:numberWords
    ...
    %also, to improve accuracy, let's ignore 1 and 2 letter words 
    if length(currentWord) > 2
        output = [output; {currentWord}]; %3+ letter words get analyzed
    else
        output = output;
end
...
for index = 1:length(output)
   ...
   elseif isempty(strmatch(currentWord,badArray(2:length(badArray),1),'exact'))==0 %is it bad?
       ...
       confusedScore = confusedScore + round(length(currentWord)/length(output));
